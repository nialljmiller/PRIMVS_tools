...
feature_weights = {
		...
		'eta': 0.5,
		'eta_e': 0.5,
		'z_med_mag-ks_med_mag': 3.5,
		'y_med_mag-ks_med_mag': 3.5,
		'j_med_mag-ks_med_mag': 3.5,
		'h_med_mag-ks_med_mag': 3.5,
		'med_BRP': 0.5,
		'range_cum_sum': 0.1,
		'max_slope': 0.1,
		'MAD': 0.4,
...
train_df = train_df[train_df['best_class_score'] > 0.7]
#train_df = train_df[train_df['ls_bal_fap'] < 0.0001]
...
if train:
    ...
    params = {
        ...
        'max_bin': 512,  # Max number of bins for histogram construction
    ...
    test_df['variable_probability'] = max_probabilities
    test_df['variable_confidence_metric'] = sorted_preds[:, 0] - sorted_preds[:, 1]
    test_df['variable_entropy'] = -np.sum(preds * np.log(preds + 1e-9), axis=1)  # Add a small value to avoid log(0)
    ...
    table_to_write.write('/beegfs/car/njm/OUTPUT/' + output_fits_file + 'new.fits', overwrite=True)
