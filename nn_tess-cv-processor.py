...
def find_orbital_period(time, flux, error):
    ...
    long_periods = np.linspace(0.01, 10, 100)
    ...
    try:
        ...
        high_power_idx = np.where(subtraction_power > 0.5 * subtraction_power[best_idx])[0]
...
def plot_phased_curves_comparison(time, flux, error, periods, star_name, output_dir):
    """
    Generate a three-panel plot showing the phase-folded light curve at the periods
    determined by the three NN_FAP methods (Chunk, Sliding Window, and Subtraction).
    ...
    error : array
        Error array
    periods : tuple
        Tuple containing the three best periods (chunk_period, sliding_period, subtraction_period) in days
    star_name : str
        Name of the star (for plot title)
    output_dir : str
        Directory to save the output plot
        
    Returns:
    --------
    None : Saves the plot to the specified output directory
    """
    import matplotlib.pyplot as plt
    import numpy as np
    from scipy.signal import savgol_filter
    ...
    chunk_period, sliding_period, subtraction_period = periods
    
    # Create the figure
    fig, axes = plt.subplots(3, 1, figsize=(10, 12), sharex=True)
    
    # Define method names and periods for iteration
    methods = ['Chunk Method', 'Sliding Window Method', 'Subtraction Method']
    method_periods = [chunk_period, sliding_period, subtraction_period]
    colors = ['blue', 'green', 'purple']
    
    # Process each method
    for i, (method, period, color) in enumerate(zip(methods, method_periods, colors)):
        ...
        bin_size = 100
        bin_edges = np.linspace(0, 1, bin_size + 1)
        bin_centers = 0.5 * (bin_edges[1:] + bin_edges[:-1])
        
        binned_flux = np.zeros(bin_size)
        binned_error = np.zeros(bin_size)
        
        for j in range(bin_size):
            mask = (phase >= bin_edges[j]) & (phase < bin_edges[j+1])
            if np.sum(mask) > 0:
                binned_flux[j] = np.mean(folded_flux[mask])
                binned_error[j] = np.sqrt(np.sum(folded_error[mask]**2)) / np.sum(mask)
            else:
                binned_flux[j] = np.nan
                binned_error[j] = np.nan
        
        # Plot the phase-folded light curve
        axes[i].errorbar(phase, folded_flux, yerr=folded_error, fmt='.', color=color, 
                         alpha=0.3, ecolor='lightgray', markersize=2)
        
        # Plot the binned data
        axes[i].errorbar(bin_centers, binned_flux, yerr=binned_error, fmt='o', 
                         color=color, alpha=0.7, markersize=4)
        
        # Add a second cycle
        axes[i].errorbar(phase + 1, folded_flux, yerr=folded_error, fmt='.', 
                         color=color, alpha=0.3, ecolor='lightgray', markersize=2)
        axes[i].errorbar(bin_centers + 1, binned_flux, yerr=binned_error, fmt='o', 
                         color=color, alpha=0.7, markersize=4)
        
        # Try to fit a smoothing spline or savgol filter for the binned data
        try:
            # Filter out NaN values
            nan_mask = np.isnan(binned_flux)
            if not all(nan_mask):
                x_valid = bin_centers[~nan_mask]
                y_valid = binned_flux[~nan_mask]
                ...
                if len(y_valid) > 10:
                    ...
                    if window_length > 3:
                        ...
                        axes[i].plot(x_valid, y_smooth, '-', color=color, linewidth=2)
                        axes[i].plot(x_valid + 1, y_smooth, '-', color=color, linewidth=2)
        except Exception as e:
            print(f"Error in smoothing for {method}: {e}")
        
        # Set axis labels and title
        axes[i].set_ylabel('Relative Flux')
        axes[i].set_title(f'{method} (P = {period*24:.6f} h)')
        axes[i].grid(True, alpha=0.3)
        
        # Calculate and annotate SNR
        try:
            mean_flux = np.nanmedian(binned_flux)
            depth = mean_flux - np.nanmin(binned_flux)
            noise = np.nanmedian(binned_error)
            snr = depth / noise if noise > 0 else 0
            
            axes[i].annotate(f'SNR: {snr:.2f}', xy=(0.02, 0.92), xycoords='axes fraction',
    ...
    # Set overall title
    plt.suptitle(f'{star_name} - Phase-Folded Light Curves Comparison', fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    
    # Save the figure
    safe_name = star_name.replace(' ', '_').replace('/', '_')
    plt.savefig(os.path.join(output_dir, f"{safe_name}_phased_comparison.png"), dpi=300, bbox_inches='tight')
    plt.close()
...
def analyze_cv_target(tic_id, common_name, cv_type, output_dir, cycles):
    ...
    # Generate the three-panel phase-folded plot
    plot_phased_curves_comparison(time, flux, error, periods, common_name, target_dir)
