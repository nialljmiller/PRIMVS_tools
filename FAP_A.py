...
ax1.plot(a_ceph_df['A_sn'] / 10, a_ceph_df['A_FAPS_BAL'], 'rx', alpha=0.05, ms=2, rasterized=True)
ax1.plot(a_eb_df['A_sn'] / 10, a_eb_df['A_FAPS_BAL'], 'g+', alpha=0.05, ms=2, rasterized=True)
ax1.plot(a_rr_df['A_sn'] / 10, a_rr_df['A_FAPS_BAL'], 'bD', alpha=0.05, ms=2, rasterized=True)
ax1.plot(a_yso_df['A_sn'] / 10, a_yso_df['A_FAPS_BAL'], 'ys', alpha=0.05, ms=2, rasterized=True)
ax1.plot(a_cv_df['A_sn'] / 10, a_cv_df['A_FAPS_BAL'], 'mp', alpha=0.05, ms=2, rasterized=True)

ax1.plot(a_ceph_df['A_sn'] / 10, a_ceph_df['A_FAPS_BAL_Smooth'], 'r-', alpha=0.8, ms=2, rasterized=True)
ax1.plot(a_eb_df['A_sn'] / 10, a_eb_df['A_FAPS_BAL_Smooth'], 'g-', alpha=0.8, ms=2, rasterized=True)
ax1.plot(a_rr_df['A_sn'] / 10, a_rr_df['A_FAPS_BAL_Smooth'], 'b-', alpha=0.8, ms=2, rasterized=True)
ax1.plot(a_yso_df['A_sn'] / 10, a_yso_df['A_FAPS_BAL_Smooth'], 'y-', alpha=0.8, ms=2, rasterized=True)
ax1.plot(a_cv_df['A_sn'] / 10, a_cv_df['A_FAPS_BAL_Smooth'], 'm-', alpha=0.8, ms=2, rasterized=True)

ax1.set(ylabel='Baluev FAP', yscale='log', xscale='linear', ylim=[10**-105, 1.1])#, xlim=[10, 80])
ax1.set_yticks([10**-75, 10**-50,10**-25, 1])
ax1.set_yticklabels(['$10^{-75}$', '$10^{-50}$','$10^{-25}$',  '1.0s'])
ax1.xaxis.tick_top()

ax2.plot(a_ceph_df['A_sn'] / 10, a_ceph_df['A_FAPS_NN'], 'rx', alpha=0.05, ms=2, rasterized=True)
ax2.plot(a_eb_df['A_sn'] / 10, a_eb_df['A_FAPS_NN'], 'g+', alpha=0.05, ms=2, rasterized=True)
ax2.plot(a_rr_df['A_sn'] / 10, a_rr_df['A_FAPS_NN'], 'bD', alpha=0.05, ms=2, rasterized=True)
ax2.plot(a_yso_df['A_sn'] / 10, a_yso_df['A_FAPS_NN'], 'ys', alpha=0.05, ms=2, rasterized=True)
ax2.plot(a_cv_df['A_sn'] / 10, a_cv_df['A_FAPS_NN'], 'mp', alpha=0.05, ms=2, rasterized=True)

ax2.plot(a_ceph_df['A_sn'] / 10, a_ceph_df['A_FAPS_NN_Smooth'], 'r-', alpha=0.8, ms=2, label='Type 1', rasterized=True)
ax2.plot(a_eb_df['A_sn'] / 10, a_eb_df['A_FAPS_NN_Smooth'], 'g-', alpha=0.8, ms=2, label='Type 2', rasterized=True)
ax2.plot(a_rr_df['A_sn'] / 10, a_rr_df['A_FAPS_NN_Smooth'], 'b-', alpha=0.8, ms=2, label='Type 3', rasterized=True)
ax2.plot(a_yso_df['A_sn'] / 10, a_yso_df['A_FAPS_NN_Smooth'], 'y-', alpha=0.8, ms=2, label='Type 4', rasterized=True)
ax2.plot(a_cv_df['A_sn'] / 10, a_cv_df['A_FAPS_NN_Smooth'], 'm-', alpha=0.8, ms=2, label='Type 5', rasterized=True)

ax2.set(xlabel=r'$A/\bar{\sigma}$', ylabel='NN FAP', yscale='log', ylim=[10**-3,1.1])
#ax2.set(xlabel=r'$A/\bar{\sigma}$', ylabel='NN FAP', yscale='log', xscale='linear')#, xlim=[10, 80])
ax2.set_yticks([10**-3, 10**-2, 10**-1, 1])
ax2.set_yticklabels(['$10^{-3}$', '$10^{-2}$', '$10^{-1}$', '1.0'])



plt.legend()
fig.tight_layout()
plt.savefig('/home/njm/FAPS/A/A_faps_log.pdf', bbox_inches='tight')
