...
class PrimvsCVFinder:
    ...
    def train_two_stage_classifier(self):
        ...
        This version uses a small tuning subset, cross-validated stacking, probability
        calibration, and a meta-model with less aggressive regularization to get the best of both worlds.
        """
        import logging
        import numpy as np
        import pandas as pd
        ...
        # Use cross-validated predictions (oof) as features for the meta-model
        skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
        trad_oof = cross_val_predict(calibrated_trad, X_trad_train, y_train, cv=skf, method='predict_proba')[:, 1]
        if calibrated_emb is not None:
            emb_oof = cross_val_predict(calibrated_emb, X_emb_train, y_train, cv=skf, method='predict_proba')[:, 1]
        else:
        ...
        meta_model = xgb.XGBClassifier(
            n_estimators=100,
            max_depth=3,  # bumped up a bit to allow more flexibility
            learning_rate=0.1,
            objective='binary:logistic',
            scale_pos_weight=pos_weight,
            n_jobs=-1,
